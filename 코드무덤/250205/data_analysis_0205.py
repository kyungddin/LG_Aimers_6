...
train = X_train_encoded.iloc[:, 52:63] # 본인 관련 데이터만 슬라이싱
train = train.join(y)
...
# 또한 알 수 없음 관련 데이터가 너무 적어서 이는 그냥 날려버릴 필요 있음
...
# 정자의 경우 미할당과 배우자 및 기증 제공은 데이터가 적어 의미 없다고 판단
# 또한 정자는 오히려 기증 제공에서 성공률이 떨어지는 모습을 보임
# 정자는 난자에 비해 기증에 의미가 크게 없는 듯
...
# 정자는 나이에 상관없이 성공률이 일정한 것으로 보아 의미 없는 데이터일 확률이 높아 보인다
